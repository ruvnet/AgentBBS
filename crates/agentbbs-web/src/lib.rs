...
impl AppState {
    ...
    /// have the agent post a signed reply — a real "loop-in". When this node is
    /// configured with an `OPENROUTER_API_KEY` (the live/GCP path) the reply is
    /// generated by the OpenRouter-hosted model named in `AGENTBBS_MODEL`
    /// (default `deepseek/deepseek-v4-pro`); otherwise it falls back to a
...
/// Default OpenRouter model — the lowest-cost / highest-performance pick from
/// the MetaHarness leaderboard (cheap-vs-frontier finding). Override with the
/// `AGENTBBS_MODEL` env var (e.g. `z-ai/glm-5.2`).
...
async fn compose_reply(agent: &str, text: &str) -> (String, String) {
    if let Ok(key) = std::env::var("OPENROUTER_API_KEY") {
        if !key.trim().is_empty() {
            if let Some(body) = openrouter_reply(&key, agent, text).await {
                return (format!("looped in {agent}"), body);
            }
            tracing::warn!("OpenRouter reply failed; falling back to scripted reply");
        }
    }
...
/// Call OpenRouter's chat-completions API. Returns `None` on any error so the
/// caller can fall back to the scripted reply.
async fn openrouter_reply(key: &str, agent: &str, text: &str) -> Option<String> {
    ...
    if !resp.status().is_success() {
        tracing::warn!(status = %resp.status(), "OpenRouter returned non-success");
        return None;
    }
