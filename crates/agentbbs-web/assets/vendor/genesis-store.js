...
const LS = {
  ...
  drafts: 'agentbbs.genesis.drafts', // ADR-0049 — agent-drafted replies awaiting human review
};
...
// Content-addressed draft id (ADR-0049) — mirrors agentbbs-core draft.rs
// Draft::content_bytes exactly (domain "agentbbs.draft.v1"), even though a
// genesis draft never crosses the wire as raw JSON to be re-verified; kept
// byte-parity-clean as a matter of house style (JS↔Rust id formulas always
// match in this codebase).
function draftBytes({ target, agent, body, createdAt }) {
  const fields = [target, agent, body, createdAt].map((f) => enc.encode(f || ''));
  const chunks = [enc.encode('agentbbs.draft.v1\n')];
...
export const store = {
  ...
  decisions() {
    ...
    let recorded = [];
    try { recorded = JSON.parse(localStorage.getItem('agentbbs.decisions') || '[]'); } catch (_) { recorded = []; }
    return { decisions: [...recorded, ...seed] };
  },
  // Record a new decision (ADR-0045), Ed25519-signed in-browser; stored locally.
  async recordDecision(seedHex, { title, decision, rationale, board = 'general' }) {
    if (!title || !decision) return { ok: false, error: 'title and decision are required' };
    const signed = await BBS.signPost(seedHex, { board: 'decisions', body: `${title}\n${decision}\n${rationale || ''}`, handle: 'you' });
    const rec = { id: signed.signature.slice(0, 16), title, decision, rationale: rationale || '', board, decided_by: signed.author.slice(0, 8), decided_at: signed.created_at };
    let all = []; try { all = JSON.parse(localStorage.getItem('agentbbs.decisions') || '[]'); } catch (_) {}
    all.unshift(rec); localStorage.setItem('agentbbs.decisions', JSON.stringify(all.slice(0, 50)));
    return { ok: true, rec };
  },
  ...
  // ---- Agent Inbox (ADR-0049): agent-drafted replies await human review ----
  // A Draft is never signed/posted itself — just an unsigned candidate sitting
  // in localStorage until you edit/send/discard it (local-only, like DMs).

  // Draft a reply for `target` (board or dm:<peer>) by asking `agent` to
  // respond to `context` (the inbound content). Scans `context` with the
  // SAME guard the post path uses (ADR-0046) BEFORE drafting — fail-closed
  // on malicious; suspicious still drafts but is flagged for extra scrutiny.
  async draftReply(target, agent, context, inReplyTo = null) {
    const scan = scanPost(context);
    if (scan.level === 'malicious') {
      return { ok: false, error: 'draft refused: ' + scan.reasons.join('; ') };
    }
    const flagged = scan.level === 'suspicious';
    const r = await this.agentReply(agent, context);
    // Millisecond precision (NOT BBS.rfc3339()'s whole-second format, which is
    // deliberately used for MESSAGE signing parity with the server) — a draft
    // never crosses the wire as raw JSON to be re-verified, so it has no
    // cross-system byte-parity requirement, and whole-second resolution let
    // two real drafts to the same agent within the same second collide on id.
    const created_at = new Date().toISOString();
    const id = blake3hex(draftBytes({ target, agent: r.handle, body: r.body, createdAt: created_at }));
    const all = readJSON(LS.drafts, []);
    const existing = all.find(d => d.id === id);
    if (existing) return { ok: true, draft: existing }; // true content+ms collision: return its REAL current state
    const draft = { id, target, in_reply_to: inReplyTo, agent: r.handle, subject: '', body: r.body, created_at, status: 'pending', flagged };
    all.unshift(draft);
    writeJSON(LS.drafts, all);
    logEvent('draft.create', `@${r.handle} drafted a reply for #${target}`);
    return { ok: true, draft };
  },
  // Drafts still awaiting a decision (pending/edited), newest first.
  pendingDrafts() {
    return readJSON(LS.drafts, []).filter(d => d.status === 'pending' || d.status === 'edited');
  },
  // A human revises the draft body before sending (marks it edited).
  editDraft(id, newBody) {
    const all = readJSON(LS.drafts, []);
    const d = all.find(x => x.id === id && (x.status === 'pending' || x.status === 'edited'));
    if (!d) return { ok: false, error: 'draft not found or already decided' };
    d.body = newBody; d.status = 'edited';
    writeJSON(LS.drafts, all);
    return { ok: true };
  },
  // Send: sign client-side under YOUR key (never the agent's — ADR-0016) and
  // post via the normal signed-post path, which already runs the same
  // postguard scan as the pre-send "verifier" pass. The agent's handle is
  // preserved as cosmetic attribution. `bodyOverride`, if given (e.g. the
  // live value of an edit textarea the human just typed in), is applied
  // atomically before sending so a pending in-progress edit is never lost.
  async sendDraft(seedHex, id, bodyOverride = null) {
    const all = readJSON(LS.drafts, []);
    const d = all.find(x => x.id === id && (x.status === 'pending' || x.status === 'edited'));
    if (!d) return { ok: false, error: 'draft not found or already decided' };
    if (bodyOverride !== null && bodyOverride !== d.body) d.body = bodyOverride;
    const r = await this.post(seedHex, { board: d.target, body: d.body, handle: d.agent, parent: d.in_reply_to });
    if (!r.ok) return r;
    d.status = 'sent';
    writeJSON(LS.drafts, all);
    logEvent('draft.sent', `sent draft to #${d.target}`);
    return { ok: true };
  },
