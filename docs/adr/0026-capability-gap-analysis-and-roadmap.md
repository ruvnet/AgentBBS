...
| G5 | **Federation auto-sync** — peer discovery, signed board snapshots for bootstrap, CRDT/gossip convergence (today: manual node URL) | ◐ signed board snapshots ✓ (`BoardSnapshot` payload + `make_snapshot`, fail-closed per-message verify); peer discovery + gossip/CRDT pending | 0007/0017 | P2 |
