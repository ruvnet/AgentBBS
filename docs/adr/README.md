...
| [0025](0025-messaging-system-bridges-slack-teams.md) | Messaging-system bridges (Slack, Teams) via a federation peer — a bridge node is a first-class peer holding an Ed25519 bridge key (per-source subkeys) mapping `channel ↔ board`; inbound external messages re-signed by the bridge + marked `bridged` (verify the bridge, not the human); Slack via Socket Mode, Teams via Workflows (outbound) + Azure Bot/RSC (inbound); loop-guard + PII-scan/opt-in egress | Proposed |
